            ...
            cardType = <>
                {isLoading ? (
                    ...
                    <div data-aos="fade-up" className='grid gap-[10px] w-full card-cat sm:mb-[10px] max-w-[190px]' data-id={item?.id ?? null}>
                        ...
                        <div className='grid gap-[7px] mt-[2px]'>
                            <Link href={`/${item?.attributes?.main_categories?.data[0]?.attributes?.Slug.toLowerCase() ?? null}/${item?.attributes?.Slug == null ? item?.name?.toLowerCase().replace(/ /g, '-') ?? null : item?.attributes?.Slug ?? null}`}>
