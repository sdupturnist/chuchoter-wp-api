                ...
                {isLoading ? (
                    ...
                    <>
                       ...
                       <div data-aos="fade-up" className={`[&>*]:text-${currentTheme}-100 grid gap-[10px] w-full card-cat sm:mb-[10px] mb-2`} data-id={item?.id ?? null} data-review={review?.length}>
                         ...
                         <Link className='block' href={`/${item?.acf?.main_categories && item?.acf?.main_categories?.toString().replace(/-ar/g, '').replace(/-en/g, '').replace(/-/g, '-').toLowerCase()}/${item?.name?.toLowerCase().replace(/ /g, '-')}/${language}`}>
                                 <Images
                                        width="170"
                                        height="170"
                                        quality={100}
...
{item?.regular_price  &&   <span className={`text-${currentTheme}-100 flex gap-[8px]`}>
                                {item?.sale_price && <del className='text-[14px] font-light'>
                                    {item?.regular_price}  {generalTranslations.qr[language]}
                                </del>
                                }
                               
                              <span className='text-[14px] font-semibold'>
                                    {item?.sale_price || item?.price}
                                    {!item?.sale_price || !item?.price && <span>{item?.regular_price ?? null}</span>} {generalTranslations.qr[language]} {item?.acf?.unit && <span className='text-[11px] font-light uppercase '>/ {item?.acf?.unit}</span>}
                                </span>
                            </span>
    }
      ...
      {item?.fea_data &&  <Images
        ...
        default:
            ...
            cardType = <div  className='grid gap-[10px] w-full card-cat sm:mb-[10px] max-w-[190px]' data-id={item?.id ?? null}  data-review={review?.length}>
            <div className='relative overflow-hidden'>
            <Link className='block' href={`/${item?.acf?.main_categories && item?.acf?.main_categories?.toString().replace(/-ar/g, '').replace(/-en/g, '').replace(/-/g, '-').toLowerCase()}/${item?.name?.toLowerCase().replace(/ /g, '-')}/${language}`}>
                    <Images
                        width="170"
